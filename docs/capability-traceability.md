...
Traceability matrices link capabilities to hardware, packages, providers, safety rules, and
verification status.
...
Columns: Hardware Component | Used By | Source Location | Capability | Provider | Verified | Safety
Rule
...
Columns: Capability | Required By | Provided By | Hardware | Package | Provider | Safety Rule |
Status

## Package contributions

Packages register capabilities in the capability registry (e.g. `spanda-nav` → `obstacle_avoidance`,
`spanda-gps` → `gps_navigation`).
