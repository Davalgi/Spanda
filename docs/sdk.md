...
|-----|---------|----------|--------|
| Rust | [`spanda-sdk`](https://crates.io/crates/spanda-sdk) on crates.io | P0 | Stable (v0.5.9 — publish via `crates-sdk-v0.5.9`) |
| Python | [`spanda-sdk`](https://pypi.org/project/spanda-sdk/) on PyPI | P1 | Stable (v0.5.9 — publish via `sdk-python-v0.5.9`) |
| TypeScript | [`@davalgi-spanda/sdk`](https://www.npmjs.com/package/@davalgi-spanda/sdk) on npm | P2 | Stable (v0.5.9 published) |
...
Install failures (PEP 668 on Python, `cargo add` in the Spanda monorepo): [troubleshooting.md —
Official SDK install](./troubleshooting.md#official-sdk-install).

Maintainers: [Publishing SDKs](sdk-publishing.md) (`crates-sdk-v*`, `sdk-python-v*`, `npm-sdk-v*`,
`desktop-v*` tags) · [Control Center versioning](control-center-versioning.md).
...
You do **not** need all three in one project. Pick the package that matches the language your app is
written in:
...
Install issues: [troubleshooting.md — Official SDK
install](./troubleshooting.md#official-sdk-install).
...
- [Cognitive & Resilience domain clients](cognitive-resilience-architecture.md#sdk-and-api) —
...
- **Simulation / replay:** Pass `"execute": true` on `POST /v1/programs/simulation` to run the
  driver; replay supports `"deterministic": true` and `"playback": true`. Default remains
  inspect-only metadata.
- **Local file paths:** Program endpoints resolve paths relative to Control Center `--config`
  project root.
- **Pool vs program readiness:** `POST /v1/readiness/run` remains device-pool impact; use `POST
  /v1/programs/readiness` for CLI-equivalent program scoring.
