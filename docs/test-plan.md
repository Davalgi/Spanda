...
| Cognitive & Resilience | reflex/homeostasis/immunity/fusion/memory CLI, entity profiles, REST+gRPC, domain SDK clients | `crates/spanda-autonomy/tests/bio_inspired.rs`, `crates/spanda-autonomy/tests/cognitive_resilience_integration.rs`, `scripts/cognitive_resilience_smoke.sh`, cross-interface autonomy checks |
...
| Control Center OAuth callbacks | `GET /admin/oauth/{oidc,slack}/callback` landing pages + PKCE on OIDC authorize | `crates/spanda-api/tests/admin_oauth_tests.rs`, Administration tab OAuth wizards |
