...
| Twin Cloud SaaS | push/pull/list/sync/import-replay + history + gRPC + RBAC | `crates/spanda-twin-cloud`, `scripts/twin_cloud_unified_path.sh`, `scripts/twin_cloud_stable_promotion_gate.sh`, `cargo test -p spanda-api twin_cloud`, `grpc_tests` (`grpc_twin_cloud_endpoints_with_patrol_program`) |
