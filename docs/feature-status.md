...
| **Enterprise operations (LATER)** | WebSocket telemetry SDK, OTLP trace export to Jaeger — **Experimental**; Compliance export UI, Digital Thread, Executive dashboards, PDF reporting — **Experimental** (E4); Tauri desktop (`@spanda/control-center-desktop`) — **Experimental** (scaffold) |
