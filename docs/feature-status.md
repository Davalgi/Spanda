...
| **Unified Entity Model** | **Experimental** → **Stable** via [entity-model-stable-promotion.md](./entity-model-stable-promotion.md) | `EntityRegistry` projects fleet, devices, humans, providers, packages into entity graph; Control Center Entities tab with read/write mutations; CI `entity_model_smoke.sh` (REST + TypeScript + Python + Rust SDK); promotion gate `entity_model_stable_promotion_gate.sh` — [entity-model.md](./entity-model.md) |
