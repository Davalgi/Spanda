...
| **Showcase demos** | `spanda demo {rover,safety,verify,fleet,health,readiness,assurance}`; `examples/showcase/*` |
...
| **Self-healing & recovery** | `recovery_policy`, recovery planner, validation gates, audit/traceability; CLI `heal`, `recover`, `recovery-report`, `sim --inject-failure`; `spanda demo` showcase paths |
...
| self-healing & recovery (static + CLI) | **Stable** | Recovery planner, validation gates, audit; no runtime auto-modification |
