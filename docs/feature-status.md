...
| **Distributed decisions** | Brain/spinal-cord/reflex layers; `decision_tree`, `offline_policy`, entity `local_decision_authority`; escalation, consensus, conflict resolution; CLI `spanda decision *`; REST `/v1/decisions*`; Control Center Decisions tab; `examples/showcase/distributed_decisions/` |
