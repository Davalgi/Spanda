...
Completed incremental refactor plan for Platform Architecture v2.0 — **all production upward waivers
eliminated** (Phase 8).
...
Validation: `python3 scripts/validate_architecture.py --check-manifest-sync` and `python3
...
**Success:** No production `spanda-driver` ↔ `spanda-certify` cycle; dev-test compile path may still
use `spanda-driver` from certify tests only.
...
**Closed waivers:** `ARCH-004`–`ARCH-012`, `ARCH-214`, `ARCH-213`. **SCC:** `ARCH-SCC-001`,
`ARCH-C02`, `ARCH-C03` dissolved (production graph acyclic).

**Remaining:** None. All interpreter upward edges and platform-service composition edges closed in
Phases 3–8.

**Success:** Interpreter depends on core platform + runtime only; services injected at interface
layer.
...
**Resolution:** Production `[dependencies]` graph is already acyclic (`driver` and `readiness` are
not in the same SCC). The former `ARCH-SCC-001` waiver was an artifact of counting
`[dev-dependencies]` in `validate_architecture.py`. CI now tracks production deps only;
`circular_dependency_waivers` is empty.
...
**Closed waivers (Phase 5b):** `TS-ARCH-001`, `TS-ARCH-007`–`TS-ARCH-020`,
`TS-ARCH-029`–`TS-ARCH-037`.
...
**Target:** Subsystems emit `spanda_audit::PlatformEvent` envelopes and persist via
...
**Result:** 16 → **0** production upward waivers. Architecture manifest cleaned of all orphan
entries (`ARCH-002`, `ARCH-015`, `ARCH-016`, `ARCH-023`, `ARCH-027`–`ARCH-036`).
...
Goal for v2.1: reduce Rust upward waivers by 25% (**achieved**). ~~Goal for v3.0: eliminate
`ARCH-SCC-001`.~~ **Done** — production graph has zero SCCs. ~~Goal for v3.1: eliminate all
remaining 16 upward waivers.~~ **Done** — 0 production upward waivers as of Phase 8.
