...
Spanda provides first-class kill switch support for emergency stops that bypass noncritical task
queues, preempt AI tasks, stop actuators, and record audit events.

**Examples:**

- [`examples/features/kill_switch.sd`](../examples/features/kill_switch.sd) — declaration + `on
  kill_switch` handler
- [`examples/security/remote_signed_kill_switch.sd`](../examples/security/remote_signed_kill_switch.sd)
  — `remote_signed` + identity
...
- Remote kill switches require `remote_signed` and verified commands
  (`RunOptions.kill_switch_signature` JSON at runtime)
- `spanda verify` reports an **error** when `remote_signed` is set without signed communication
  policy (Phase 35)
...
- [Verification diagnostics](./verification-diagnostics.md) — `--verification-json`, kill-switch
  severity
