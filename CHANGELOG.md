...
- **Control Center enhancement backlog (shipped):** new tabs — Telemetry (WebSocket), Trends, Continuity, Fleet map, Reports, Playground (WASM), Marketplace, Chaos; SRE observability bridge (Grafana embed via `SPANDA_GRAFANA_URL`); config history + deploy gate modal; bulk device actions; OTA rollout progress; decision policy editor; Smart Spaces floor-plan SVG; analytics trust graph + certification pack download; Twin Cloud usage; OIDC/Slack admin wizards (experimental); offline dashboard cache; plugin bundle loader; Humans collaboration graph + AR annotate; entity graph default view; desktop `spawn_control_center_api` command. REST: `/v1/fleet/map`, `/v1/config/history`, `/v1/deploy/gate`, `/v1/admin/oidc`, `/v1/admin/slack`, `/v1/chaos/*`, `/v1/plugins/control-center/{plugin}/bundle`. **gRPC server reflection** enabled.
