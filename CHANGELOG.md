...
- **Unified Entity Model promoted to Stable:** `docs/feature-status.md` and [ROADMAP.md](ROADMAP.md) Pillar 0 updated; SDKs **0.4.1** already published on crates.io, PyPI, and npm (`crates-sdk-v0.4.1`, `sdk-python-v0.4.1`, `npm-sdk-v0.4.1`). Removed `spanda-certify` from `spanda-driver`; certification proof summaries live in `spanda-ota::build_deploy_plan`; runtime gate moved to `spanda-assurance` (`certify-runtime` feature) invoked from `spanda-interpreter`; strict verify certification merged in `spanda-config::verify_with_system_config`. Waivers `ARCH-001` and `ARCH-C01` removed.
