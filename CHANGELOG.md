...
- **Persistent telemetry store:** device/sensor/topic/heartbeat/health JSONL storage; `device_heartbeat` for IoT/fleet/deploy liveness; `robot_id` tagging; `spanda fleet run --persist-telemetry`; golden path `scripts/telemetry_store_golden_path.sh`; `spanda man telemetry`
