...
- **Bio-inspired autonomy Phase A–C:** OpenAPI parity for autonomy routes; entity registry auto-profiles; TypeScript/Python SDK methods; Control Center live REST panels; README golden tests; runtime entity context from health/trust; recovery↔autonomy confidence bridge; in-process reflex trace buffer wired from kill switch and reflex decision trees; AST preview slots for `homeostasis_policy` / `attention_policy`; `scripts/bio_inspired_autonomy_smoke.sh`.
