...
use spanda_governance::{
    ...
    run_compliance_check, validate_governance, verify_deployment, CertificationRecord,
    CertificationReport, CertificationStatus, CertificationStore, ValidationOptions,
};
use std::env;
use std::process;

fn load_registry() -> (
    spanda_config::EntityRegistry,
    spanda_config::ResolvedSystemConfig,
) {
...
pub fn governance_report_dispatch(args: &[String]) {
    ...
    println!(
        "{}",
        format_governance_report(&compliance, &validation, json)
    );
        ...
        if json {
            ...
            if !profile.required_capabilities.is_empty() {
                println!(
                    ...
                    profile.required_capabilities.join(", ")
                );
            }
...
pub fn governance_framework_dispatch(args: &[String]) {
    ...
    if json {
        ...
        println!("================================");
        println!(
            "Capabilities: standards awareness, compliance validation, deployment governance,"
        );
...
pub fn certification_list_dispatch(args: &[String]) {
    ...
    if json {
        println!(
            "{}",
            serde_json::to_string_pretty(&certs).unwrap_or_default()
        );
...
pub fn certification_inspect_dispatch(args: &[String]) {
    ...
    if json {
        ...
        if let Some(gov) = entity.governance.as_ref() {
            println!(
                "Autonomy Level: {}",
                gov.autonomy_level.as_deref().unwrap_or("-")
            );
            println!(
                "Deployment Profile: {}",
                gov.deployment_profile.as_deref().unwrap_or("-")
            );
            println!(
                "Operational Maturity: {}",
                gov.operational_maturity.as_deref().unwrap_or("-")
            );
            println!(
                "Certification Status: {}",
                gov.certification_status.as_deref().unwrap_or("-")
            );
            println!("Risk Level: {}", gov.risk_level.as_deref().unwrap_or("-"));
            println!(
                "Responsible Person: {}",
                gov.responsible_person.as_deref().unwrap_or("-")
            );
            println!(
                "Deployment Owner: {}",
                gov.deployment_owner.as_deref().unwrap_or("-")
            );
            println!(
                "Mission Owner: {}",
                gov.mission_owner.as_deref().unwrap_or("-")
            );
            println!(
                "Emergency Contact: {}",
                gov.emergency_contact.as_deref().unwrap_or("-")
            );
            println!(
                "Escalation Contact: {}",
                gov.escalation_contact.as_deref().unwrap_or("-")
            );
