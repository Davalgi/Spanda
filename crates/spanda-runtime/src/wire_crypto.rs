...
impl WireCryptoSession {
    ...
    pub fn from_material(material: &str) -> Self {
        // Derive the AES-256-GCM key from configured cert/key session material.
    ...
    pub fn encrypt(&self, plaintext: &[u8]) -> Result<Vec<u8>, String> {
        // Encrypt plaintext with a random 12-byte nonce prepended to the ciphertext.
        let cipher =
    ...
    pub fn decrypt(&self, data: &[u8]) -> Result<Vec<u8>, String> {
        // Decrypt a nonce-prefixed AES-GCM ciphertext produced by `encrypt`.
        if data.len() < 13 {
