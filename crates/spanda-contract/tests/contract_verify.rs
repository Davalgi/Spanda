...
fn contract_verify_warehouse_passes() {
    ...
    assert!(
        ...
        "expected mission_plan contract"
    );
    assert!(
        report
            .contracts
            .iter()
            .any(|c| c.name == "WarehouseInventoryScan"),
        "mission plan name"
    );
    assert!(report
        .checks
        .iter()
        .any(|c| c.name == "safety_clause" && c.passed));
    assert!(report
        .checks
        .iter()
        .any(|c| c.name == "continuity_clause" && c.passed));
    assert!(report
        .checks
        .iter()
        .any(|c| c.name == "recovery_clause" && c.passed));
    assert!(
        report.passed,
        "expected contract verification to pass: {:?}",
        report.issues
    );
}
